...
void mostrarOcupacao(char **mx){
    ...
    printf("\n----------------------------------");
    printf("\nCadeiras meia vendida...: %d.\nCadeiras inteira vendida: %d.",m,in);
    printf("\n----------------------------------");
    printf("\n           Fim Ocupação           ");
    printf("\n----------------------------------");
...
int outro(char c[13]){
    printf("\n%s de outro ingresso (0:sim; 1:nao)?\t",c);
    int o=lerOp();
    //while(!(lerOp()==1||lerOp()==0))printf("\nVALOR INVALIDO!ESCOLHA NOVAMENTE!\nVender outro ingresso (0:nao; 1:sim)?");
    while(o>1 || o<0){
        printf("\nVALOR INVALIDO!ESCOLHA NOVAMENTE!\n%s outro ingresso (0:sim; 1:nao)?",c);
        o=lerOp();
    }
    if(o==1){
        if (c == "Venda"){
        printf("\n---------------------------------------------------------");
        printf("\n                 Fim Venda de Ingressos                  ", c);
        printf("\n---------------------------------------------------------");
        } else {
            printf("\n---------------------------------------------------------");
            printf("\n              Fim Cancelamento de Ingressos              ", c);
            printf("\n---------------------------------------------------------");            
        };
    };
    return o;
...
int Vender(char **mx,int i, int j){
    ...
    if (c == 'i' || c == 'I') {
        ...
        printf("\n(Fileira %d, coluna %d).",(i+1), (j+1));
        return outro("Venda"); // Adicionamos o ponto e vírgula
    } else if (c == 'm' || c == 'M') {
...
int cancelarIngresso(char **mx){
    printf("\nDigite 0 para voltar ao Menu!\nDigite um numero do assento para o cancelamento (zero encerra):\t");
    int op = lerOp();
    if (op == -1) {
        printf("\nNao é um inteiro! Digite um numero inteiro.\n"); // sair
        return 0;
    } else if (op==0) { // Escolheu sair
        printf("\nVoltando ao menu\n"); // sair
        return 1;
    }else if (!(op >= 1 && op <= 120)) { // Ajustando a faixa para 1 a 120
        printf("\nValor deve estar entre 0 e 120.\n"); // sair
        return 0;
    }
